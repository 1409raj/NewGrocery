...
public class ProductController {
	...
	public @ResponseBody ResponseEntity<Domain<List<ProductsEntity>>> getProductByCategoryId(
	...
	public @ResponseBody ResponseEntity<Domain<List<ProductsEntity>>> getProductByCategoryName(
	...
	public @ResponseBody ResponseEntity<Domain<Boolean>> saveCategory(
	...
	public @ResponseBody ResponseEntity<Domain<List<ProductCategoryEntity>>> getCategoryList(
	...
	public @ResponseBody ResponseEntity<Domain<ProductsEntity>> getproductAdddshowetails(@RequestParam("id") String id,
	...
	public @ResponseBody ResponseEntity<Domain<Boolean>> saveproductsForMerchant(
			@RequestParam("merchantid") String merchantId, @RequestParam("categoryid") String categoryid,
			@RequestParam("productid") String productid, @RequestParam("imageid") String imageid,
			@RequestParam("wprice") String wprice, @RequestParam("wweight") String wweight,
			@RequestParam("wunit") String wunit, @RequestParam(value = "image", required = false) MultipartFile files) {
		...
		/*
		 * Iterator<String> it=wpriceList.iterator(); while(it.hasNext()) { String
		 * newid=it.next(); productPriceEntity.setPrice(newid);
		 * productPriceEntity.setProductid(productids);
		 * productPriceEntity.setStatus("Y"); Domain<Boolean>
		 * productprice=productService.createPrice(productPriceEntity); int cpriceid =
		 * productPriceEntity.getPriceId(); }
		 * 
		 * 
		 * Iterator<String> wit=wweightList.iterator(); while(wit.hasNext()) { String
		 * newwid=wit.next(); productUnitsWeightEntity.setUnitId(wunits);
		 * productUnitsWeightEntity.setProductId(productids);
		 * productUnitsWeightEntity.setWeight(newwid); Domain<Boolean>
		 * productunitweight=productService.createunitweight(productUnitsWeightEntity);
		...
		/*
		 * try { byte[] bytes11 = files.getBytes(); File dir11 = new
		 * File(ApiConstants.image_path); if (!dir11.exists()){ dir11.mkdirs(); } File
		 * serverFile11 = new File(dir11.getAbsolutePath() + File.separator +
		 * files.getOriginalFilename());
		 * productImageEntity.setImage(files.getOriginalFilename());
		 * productImageEntity.setProductid(productids); Domain<Boolean>
		 * productImage=productService.saveproductImage(productImageEntity);
		 * cimageid=productImage.getId(); if(productImage.getHasError().equals("false"))
		 * { BufferedOutputStream stream = new BufferedOutputStream( new
		 * FileOutputStream(serverFile11)); stream.write(bytes11);
		 * 
		 * stream.close(); //logger.info("files uploaded successfully ");
		 * System.out.println("files uploaded successfully"); Domain<Boolean>
		 * updateids=productService.updateAllProductidaftersave(proid, proprice,
		 * proweight, proimgid);
		 * 
		 * }
		 * 
	...
	public @ResponseBody ResponseEntity<Domain<Boolean>> saveProduct(@RequestParam("merchantid") String merchantId,
			...
			@RequestParam("unitid") String unitid, @RequestParam("unitweight") String unitweight,
...

	{
		...
		try {
			...
			File serverFile11 = new File(dir11.getAbsolutePath() + File.separator + fname);
			productImageEntity.setImage(fname);
			productImageEntity.setProductid(proid);
			productImageEntity.setStatus("N");
			...
			if (productImage.getHasError().equals(false)) {
				BufferedOutputStream stream = new BufferedOutputStream(new FileOutputStream(serverFile11));
		...
		for (int i = 0; i < newweightList.size(); i++) {
			...
			if (product.getHasError().equals(false)) {
				...
				Domain<Boolean> updateids = productService.updateAllProductidaftersave(proid, proprice, proweight,
						proimgid);
	...
	public @ResponseBody ResponseEntity<List<MerchantProductDetails>> getMerchantProductdetails(
			@RequestParam("mid") String mid, @RequestParam("pid") String pid) {
		int mids = Integer.parseInt(mid);
	...
	public @ResponseBody ResponseEntity<Domain<Boolean>> updateProductQuantity(
			@RequestBody List<MerchantUpdateQuantity.MerchantPurchaseProductDetailEList> merchantUpdateQuantity) {
		/*
		 * System.out.println(new Gson().toJson(merchantUpdateQuantity,
		 * MerchantUpdateQuantity.MerchantPurchaseProductDetailEList.class));
		 */
	...
	public @ResponseBody Domain<List<ProductsEntity>> updatesProductQuantity(
		...
		for (MerchantUpdateQuantity.MerchantPurchaseProductDetailEList obj : productList
				.getMerchantPurchaseProductDetailEList()) {
	...
	public @ResponseBody ResponseEntity<Domain<List<UsersEntity>>> getMerchantdetails(
			@RequestParam("unitid") String unitid, @RequestParam("productid") String productid) {
	...
	public @ResponseBody ResponseEntity<Domain<List<ProductsEntity>>> getlistbyaccbynopurchase(
	...
	public @ResponseBody ResponseEntity<Domain<List<ProductsEntity>>> getProductListByaccordingMerchantandcategory(
			@RequestParam("categoryid") String categoryid, @RequestParam("merchantid") String merchantid) {
		int ids = Integer.parseInt(categoryid);
	...
	public @ResponseBody ResponseEntity<Domain<List<ProductUnitsWeightEntity>>> getProductByProductId(
	...
	public @ResponseBody ResponseObject checkProduct(@RequestParam("cartid") String cartid,
			@RequestParam("ordercartid") String orderid, @RequestParam("productid") String productid,
			@RequestParam("weight") String weight, @RequestParam("price") String price,
			@RequestParam("unit") String unit, @RequestParam("unitweightid") String unitweightid,
			@RequestParam("merchantid") String merchantid, @RequestParam("customeruserid") String customeruserid,
			@RequestParam("quantity") String quantity) {
		...
		if (domain.getMessage().equals("true")) {
			...
			for (int i = 0; i < cartidList.size(); i++) {
				if (dao.IncreaseProductMerchanttnoOfPurchase(Integer.parseInt(productididList.get(i)),
	...
	public @ResponseBody ResponseObject saveProduct(@RequestParam("cartid") String cartid,
			@RequestParam("ordercartid") String orderid, @RequestParam("productid") String productid,
			@RequestParam("weight") String weight, @RequestParam("price") String price,
			@RequestParam("unit") String unit, @RequestParam("unitweightid") String unitweightid,
			@RequestParam("merchantid") String merchantid, @RequestParam("customeruserid") String customeruserid,
			@RequestParam("quantity") String quantity, @RequestParam("ordertype") String ordertype,
			@RequestParam("transactionid") String paymenttransacid, @RequestParam("totalprice") String totalprice) {
		Domain<TrackOrderEntity> trackentity = new Domain<TrackOrderEntity>();
		Domain<OrderPaymentEntity> orderEntity = new Domain<OrderPaymentEntity>();
		...
		for (int i = 0; i < cartidList.size(); i++) {
			...
			if (i == 0) {
				orderPaymentEntity.setOrderid(message);
				orderPaymentEntity.setMerchantid(Integer.parseInt(merchantidList.get(i)));
				orderPaymentEntity.setCustomeruserid(Integer.parseInt(customeruseridList.get(i)));
				orderPaymentEntity.setOrdertype(ordertype);
				orderPaymentEntity.setTransactionid(paymenttransacid);
				orderPaymentEntity.setOrderprice(price);
				orderPaymentEntity.setCreatedOn(date);
				orderPaymentEntity.setUpdatedOn(date);

				orderEntity = productService.orderpayment(orderPaymentEntity);
			}
			...
			/*
			 * if(dao.IncreaseProductMerchanttnoOfPurchase(Integer.parseInt(productididList.
			 * get(i)), Integer.parseInt(unitweightidList.get(i)),
			 * Integer.parseInt(merchantidList.get(i)),Integer.parseInt(quantityList.get(i))
			 * )) { System.out.
			 * println("Increase merchant product no of purchase and decrease quantity");
			 * }else { System.out.println("Not merchant product increase"); }
			 */
			/*
			 * if(i==0) { trackOrderEntity.setCrearteOn(date);
	...
	public @ResponseBody ResponseObject cnacelpaymentProduct(@RequestParam("cartid") String cartid,
			@RequestParam("ordercartid") String orderid, @RequestParam("productid") String productid,
			@RequestParam("weight") String weight, @RequestParam("price") String price,
			@RequestParam("unit") String unit, @RequestParam("unitweightid") String unitweightid,
			@RequestParam("merchantid") String merchantid, @RequestParam("customeruserid") String customeruserid,
		...
		for (int i = 0; i < cartidList.size(); i++) {
			if (dao.CanceldecreaseProductMerchanttnoOfPurchase(Integer.parseInt(productididList.get(i)),
					Integer.parseInt(unitweightidList.get(i)), Integer.parseInt(merchantidList.get(i)),
					Integer.parseInt(quantityList.get(i)))) {
	...
	public @ResponseBody ResponseEntity<Domain<List<UsersEntity>>> getNearestMerchantDetail(
			@RequestParam("customerid") String customerid, @RequestParam("radius") String radius) {
		int cids = Integer.parseInt(customerid);
	...
	public @ResponseBody ResponseEntity<Domain<ProductsEntity>> getProductListBySearch(
	...
	public @ResponseBody Domain<ProductCartHistory> updateStatus(@RequestParam("orderid") String orderid,
			@RequestParam("statusid") String statusid) {
		System.out.println(orderid);
		Domain cartHistory = new Domain();
		Date date = new Date();
		/*
		 * List<String> carthidList = Arrays.asList(carthid.split(",")); List<String>
		 * statusidList = Arrays.asList(statusid.split(","));
		 */
	...
	public @ResponseBody ResponseEntity<Domain<String>> saveCatrgoryForMerchant(
			@RequestParam("merchantid") String merchantId, @RequestParam("categoryid") String categoryId) {
	...
	public @ResponseBody ResponseEntity<Domain<List<ProductUnitsWeightEntity>>> getUnitbyProductidMerchant(
			@RequestParam("productid") String productid, @RequestParam("merchantid") String merchantid) {
		int ids = Integer.parseInt(productid);
		int mids = Integer.parseInt(merchantid);
	...
	@RequestMapping(value = "/getUpdateProductSataus", method = RequestMethod.POST)
	public @ResponseBody Domain<Boolean> updateProductStatus(@RequestParam("status") String status,
			@RequestParam("productId") Integer productId) {
		Domain<Boolean> domain = new Domain<Boolean>();

		if (dao.updateProductStatus(status)) {
			domain.setMessage("Status Successfully Updated");
			domain.setHasError(false);
		} else {
			domain.setMessage("Status not Updated.");
			domain.setHasError(true);
		}
	...
	@RequestMapping(value = "/getAllProductforAdmin", method = RequestMethod.GET)
	public @ResponseBody ResponseEntity<Domain<List<ProductsEntity>>> getProductListforAdmin() {

		Domain<List<ProductsEntity>> entity = productService.getAllProductforAdmin();
		return new ResponseEntity<Domain<List<ProductsEntity>>>(entity, HttpStatus.OK);
	}
	 ...
	 */
	@RequestMapping(value = "/getAllProductforUser", method = RequestMethod.GET)
	public @ResponseBody ResponseEntity<Domain<List<ProductsEntity>>> getCategoryList() {

		Domain<List<ProductsEntity>> entity = productService.getProductListforUser();
		return new ResponseEntity<Domain<List<ProductsEntity>>>(entity, HttpStatus.OK);
	}

	/*
	 * for updating the product status
	 */
	@RequestMapping(value = "/updatestatus", method = RequestMethod.PUT)
	public @ResponseBody ResponseEntity<Domain<Boolean>> updateProductStatus(
			@RequestParam("product_id") String productid, @RequestParam("status") String status) {
		int product_id = Integer.parseInt(productid);

		Domain<Boolean> product = productService.updateProductStatus(product_id, status);
		return new ResponseEntity<>(product, HttpStatus.OK);
	}

	@RequestMapping(value = "/updateProductStatus", method = RequestMethod.PUT)
	public @ResponseBody ResponseEntity<Domain<Boolean>> updateProductStatusForView(
			@RequestParam("product_id") String productid, @RequestParam("product_status") String p_status) {
		int product_id = Integer.parseInt(productid);
		int product_status = Integer.parseInt(p_status);
		Domain<Boolean> product = productService.updateProductStatusForView(product_id, product_status);
		return new ResponseEntity<>(product, HttpStatus.OK);

	}

	@RequestMapping(value = "/updateUserStatus", method = RequestMethod.PUT)
	public @ResponseBody ResponseEntity<Domain<Boolean>> updateUserStatus(@RequestParam("merchant_id") String m_id,
			@RequestParam("product_status") String p_status) {
		int merchant_id = Integer.parseInt(m_id);
		int product_status = Integer.parseInt(p_status);
		Domain<Boolean> product = productService.updateUserStatus(merchant_id, product_status);
		return new ResponseEntity<>(product, HttpStatus.OK);
